...
class Quiz:
    ...
    def initialize_ui(self):
        try:
            ...
            self.question_count_label.configure(text=f"QUESTION {self.question_count + 1}/{self.question_set_length}")
    ...
    def check_answer(self, button_index):
        for index in range(self.buttons_length):
            ...
            if self.buttons[index].cget("text").lower() == self.answer:
        ...
        if self.user_answer.lower() == self.answer:
            ...
            self.question_remark_label.configure(
                text=f"INCORRECT! Answer is {self.answer.upper()}",
                text_color="yellow"
            )
        ...
        if self.question_count >= self.question_set_length - 1:
            self.window.after(800, self.end_quiz)
        else:
